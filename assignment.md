...
Build APIs to create and manage orders. Required APIs:
...
1.	Scheduler Task
Create a scheduled task that automatically updates order statuses. Required API:

Scheduler logic:

•	Fetch all orders with status
•	If an order is older than 10 minutes, update it to
•	Maintain status history
•	Save scheduler execution logs
•	Protect this API using a secret key in headers Example header:
...
•	Empty state
•	Error handling
•	Auto-refresh button Example API usage:


1.	Order Status Flow
Orders should follow this status flow:

For this assignment, the scheduler must handle:
Bonus: Candidate can also handle:
after another fixed time interval. Example:
...
•	Any cloud scheduler or local cron for testing Follow a clean folder structure:

Use	for all secrets and environment variables.
